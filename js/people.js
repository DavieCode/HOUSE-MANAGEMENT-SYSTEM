...
function init_profile_uploader()
{
    ...
    var uploader = new plupload.Uploader({
        ...
        url: $("#site_url").val() + '/tenants/upload_profile_pic',
        unique_names: true,
...
function init_attachment_uploader()
{
    ...
    var uploader_a = new plupload.Uploader({
        ...
        url: $("#site_url").val() + 'tenants/upload_attachment',
        unique_names: true,
        resize: {width: 500, height: 500, quality: 100},
        max_retries: 3,
        flash_swf_url: 'Moxie.swf',
        silverlight_xap_url: 'Moxie.xap',
        multipart_params: {
            tenant_id: $("#pickfiles_a").data("tenant-id"),
            softtoken: $("#token_hash").val(),
            linker: $("#linker").val()
        },
        filters: {
    ...
    uploader_a.bind('FileUploaded', function (upldr, file, object) {
        ...
        $("#filelist_a").append("<li><a href=\"uploads/tenant-" + myData.tenant_id + "/" + myData.filename + "\" target=\"_blank\" title=\"" + myData.filename + "\"><img src=\"" + myData.icon + "\" /></a><span class=\"close remove-file\" data-file-id=\"" + myData.id + "\" title=\"Remove this file\"><i class=\"fa fa-times-circle\"></i></span></li>");
    });
