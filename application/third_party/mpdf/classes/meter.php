...
';
	...
	// VALUE Marker
	if ($value) {
	  if ($min != $low && $value < $low) { $col = '#e9573f'; }
	  else if ($max != $high && $value > $high) { $col = '#e9573f'; }
	  else { $col = '#008800'; }
...
';
	...
	<linearGradient id="Gr#e9573f" x1="0" y1="0" x2="0" y2="1" gradientUnits="boundingBox">
...
';
	...
	if ($value) {
	  ...
	  if ($optimum < $low) {
		...
		if ($value < $low) { $barcol = 'url(#Gr#e9573f)'; }
		else if ($value > $high) { $barcol = 'url(#Gr#e9573f)'; }
